...
public class Deposit extends Account{
    ...
    public BigDecimal countBalanceOverPeriod(int days, DateTime currentData) {
        ...
        for (int i = 1; i <= days; ++i){
            ...
            newBalance = newBalance.add((percent).divide(new BigDecimal(new Constants().DAYS_IN_YEAR), 10, RoundingMode.HALF_UP).divide (new BigDecimal(new Constants().ALL_PERCENTS), 10, RoundingMode.HALF_UP).multiply(newBalance));
        }
